...
1. `models/` (logistic regression, random forest, XGBoost, neural net, seed KNN) — trains on `features.build_matchup_history()` + `randomize_matchup_sides()` output
2. `bracket/simulate.py` (consumes `bracket/structure.py`, which is done)

Repo skeleton, `pyproject.toml`, config module, bracket structure module, both ingest modules, and the feature-building module are done (see Active Files). Do not assume any other module is implemented until it appears here.
...
- `tests/test_config.py`, `tests/test_bracket_structure.py`, `tests/test_kaggle_ingest.py`, `tests/test_kenpom_ingest.py`, `tests/test_build_features.py` — all verified against real legacy-project data (2024 `MNCAATourneySlots.csv`, the full 2026 Kaggle CSV set including `MTeamSpellings.csv`, and the real `kenpom_2026_raw.csv`), not just synthetic fixtures.
- `src/march_madness/{models,analysis}/` — empty `__init__.py` placeholders, not yet implemented.
