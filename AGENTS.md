...
1. `analysis/` — region strength, upsets, round-advancement counts (consumes `bracket/simulate.py`'s output, which is done). This is the last piece of Milestone 1's port besides seed clustering.
2. Port `seed_clustering.py`'s KMeans tiering into the new structure.
3. A `scripts/` entry point that wires ingest → features → models → simulate → analysis into one runnable pipeline — nothing currently ties these modules together end-to-end except manual verification scripts.

Repo skeleton, `pyproject.toml`, config module, bracket structure module, both ingest modules, the feature-building module, all five `models/`, and `bracket/simulate.py` are done (see Active Files). Do not assume any other module is implemented until it appears here.
