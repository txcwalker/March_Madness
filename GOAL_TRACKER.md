...
| — `run_pipeline.py` default model, final state 2026-07-30: `xgboost_model`. History in one line: logistic_regression → xgboost_model (accuracy 0.681, looked broken) → reverted to logistic_regression → root-caused to training-data starvation, not model choice → historical KenPom backfill (below) → xgboost_model 0.744 vs logistic_regression's 0.755, close enough that the user chose xgboost_model as live default. Open concern, not yet resolved: post-backfill, xgboost_model gives Duke a much larger title edge (67.5%) than logistic_regression did (32.2%) despite Duke/Arizona/Michigan having near-identical KenPom NetRtg — per-game calibration checks out even in the 90-100% bucket, so this isn't simple overconfidence, but it's still a real open question about how the model weighs features beyond NetRtg, not settled | done | 2026-07-30 |
